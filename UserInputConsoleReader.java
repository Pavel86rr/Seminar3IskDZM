...
public class UserInputConsoleReader implements UserInputReader {
    ...
    public UserData readUserData() throws UserDataException {
        while (true) {
            System.out.println("Введите данные в следующем порядке, разделяя их пробелом: Фамилия Имя Отчество Дата рождения (dd.mm.yyyy) Номер телефона(+7xxxxxxxxxx) Пол (f или m)");
    ...
    private long parsePhoneNumber(String phoneNumberString) throws ParseException {
        try {
            ...
            if (phoneNumberString.length()!=12) throw new ParseException("Неверный формат номера телефона");
            else {
